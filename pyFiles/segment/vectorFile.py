...
class vector(method):
    ...
    def onlyDraw(self):
        self.__del__()
        #X, Y, U, V = self.data[0][0], self.data[1][0], self.data[0][0] + self.data[1][0], self.data[0][1] + self.data[1][1]
        X, Y, U, V = self.data[0][0], self.data[1][0], self.data[1][0], self.data[1][1]
        line = self.ax.quiver(X, Y, U, V, angles= 'xy', scale_units='xy', scale=1, width = 0.003)
